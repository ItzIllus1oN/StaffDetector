...
public class ConfigScreenUI extends Screen {
    ...
    protected void init() {
        ...
        this.alertToggle = CheckboxWidget.builder(Text.of("Staff Watch Alert"), this.textRenderer)
            .pos(rightX, startY + step * 3).checked(config.spectatorAlertEnabled).build();
        this.addDrawableChild(this.alertToggle);

        // Keywords field
        int fieldY = startY + step * 4 + 10;
        this.keywordsField = new TextFieldWidget(this.textRenderer, this.width / 2 - 150, fieldY, 300, 20, Text.of("Keywords"));
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTextWithShadow(this.textRenderer, "Staff Keywords (comma separated):", this.width / 2 - 150, fieldLabelY, 0xAAAAAA);
        super.render(context, mouseX, mouseY, delta);
    }

    public static void open() {
        MinecraftClient.getInstance().execute(() ->
            MinecraftClient.getInstance().setScreen(new ConfigScreenUI())
        );
    }
}
