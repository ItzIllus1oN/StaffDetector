...
implements CallbackListener {
    ...
    public void sendHelloWorld(KeyEvent e) {
        if (e.input.isDown()) {
            ChatUtils.sendFormatted("Hello World + %s", GLFW.glfwGetKeyName((int)e.key, (int)e.scan));
        }
    }
    ...
    public void printSelf(MouseEvent e) {
        if (e.input.isDown()) {
            ChatUtils.sendMessage("target: " + String.valueOf(e.target));
        }
    }
